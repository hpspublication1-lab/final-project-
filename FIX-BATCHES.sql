...
-- 0. Shared updated_at trigger helper (this diverged DB is missing it, which is
--    why the original run failed with "function update_updated_at_column does
--    not exist"). Create it first so the triggers below work.
...
-- Make PostgREST expose the new tables immediately.
